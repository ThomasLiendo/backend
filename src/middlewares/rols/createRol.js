...
const createRol = async (req, res, next) => {
    try{
        ...
        if(typeof rol !== "string" || rol === undefined){
            throw new Error(`El Nombre del Producto debe ser unicamente texto, y has insertado ${rol === undefined ? "texto vacio": rol}`)
        } 
        const newProduct = await Rol.create({
            rol
        })
        req.body.resultado = {status:"200", respuesta:`el Rol ${rol} se ah creado exitosamente`};
        next();
